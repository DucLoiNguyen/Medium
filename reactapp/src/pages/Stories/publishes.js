...
function Publish() {
    ...
    const fetchPublications = useCallback(async (cursor = null) => {
        try {
            setLoading(true);
            const url = 'http://localhost:3030/api/post/getmypublish';
            ...
            if ( data.length === 0 ) {
                ...
                setPublications(prev => cursor ? [...prev, ...data] : data);
    ...
    const handleDeletePublication = async (itemId) => {
        try {
            await axios.delete(`http://localhost:3030/api/post/${ itemId }`, { withCredentials: true });
            setPublications(prev => prev.filter(pub => pub._id !== itemId));
            closeConfirmModal();
        } catch ( error ) {
    ...
    const lastPublicationElementRef = useCallback(node => {
        ...
        observer.current = new IntersectionObserver(entries => {
            if ( entries[0].isIntersecting && hasMore ) {
                fetchPublications(nextCursor);
            }
        }, { threshold: 0.5 });

        if ( node ) observer.current.observe(node);
    }, [loading, hasMore, nextCursor, fetchPublications]);
        ...
        <div className="divide-y divide-gray-200">
                        ...
                        <div className="flex mt-3">
                            <div className="max-w-lg">
                                ...
                                <div className="text-sm text-gray-500 mt-1">
                                    ...
                                    { item.likes && (
                                        <span
                                            className="ml-2">• { item.likes } like{ item.likes !== 1 ? 's' : '' }</span>
                                    ) }
                                </div>
                            </div>
                        </div>
            ...
            { loading && (
                <div className="px-6 py-4 text-center text-sm">
                    ...
                    <p className="mt-2 text-sm text-gray-600">Loading publications...</p>
                </div>
            ) }

            { !hasMore && publications.length > 0 && (
                <div className="px-6 py-4 text-center text-sm text-gray-500">
                    You've reached the end of your publications
                </div>
            ) }
            ...
            <ComfirmModal
                ...
                type={ 'deletestories' }
                onConfirm={ () => currentItemId && handleDeletePublication(currentItemId) }
            />
        </div>
    );
}
